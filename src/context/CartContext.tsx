...
interface CartContextType {
  cartItems: CartItem[];
  addToCart: (product: ProductType) => Promise<boolean>;
...
export function CartProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    async function loadCart() {
      if (!user) {
        ...
        const savedCart = localStorage.getItem("hanger_guest_cart");
        if (savedCart) {
          try {
  ...
  // Save guest cart to localStorage when it changes in guest mode
  useEffect(() => {
    if (!user && !isLoading) {
  ...
  const addToCart = async (product: ProductType): Promise<boolean> => {
    if (!user) {
      // Guest Mode: add directly to local state
  ...
  const clearCart = async () => {
    if (!user) {
      setCartItems([]);
      localStorage.removeItem("hanger_guest_cart");
